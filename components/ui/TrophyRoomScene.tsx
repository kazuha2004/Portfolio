...
function Node({ data, onClick }: { data: typeof ACHIEVEMENTS[0]; onClick: (data: any) => void }) {
  ...
  const lines = useMemo(() => {
    ...
    for (let i = 0; i < ACHIEVEMENTS.length; i++) {
      for (let j = i + 1; j < ACHIEVEMENTS.length; j++) {
        // Connect if distance is less than 4
...
export default function TrophyRoomScene() {
  const [activeNode, setActiveNode] = useState<typeof ACHIEVEMENTS[0] | null>(null);

  return (
    <div className="relative w-full h-[300px] sm:h-[400px] md:h-[500px] bg-[#0A0A0A] rounded-2xl overflow-hidden border border-white/5 shadow-2xl group cursor-move">
      <Canvas camera={{ position: [0, 0, 6], fov: 45 }}>
        <ambientLight intensity={0.5} />
        <ConstellationConnections hoveredNode={activeNode?.id || null} />
        {ACHIEVEMENTS.map((ach) => (
          <Node
            key={ach.id}
            data={ach}
            onClick={(data) => setActiveNode(data)}
          />
        ))}
        <OrbitControls
          enableZoom={false}
